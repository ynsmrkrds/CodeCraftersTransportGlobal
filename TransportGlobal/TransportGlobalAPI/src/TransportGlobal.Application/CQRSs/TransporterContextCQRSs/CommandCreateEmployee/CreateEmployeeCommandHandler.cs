    ...
    public class CreateEmployeeCommandHandler : IRequestHandler<CreateEmployeeCommandRequest, CreateEmployeeCommandResponse>
    {
        ...
        public Task<CreateEmployeeCommandResponse> Handle(CreateEmployeeCommandRequest request, CancellationToken cancellationToken)
        {
            ...
            if (request.VehicleID != null)
            {
                ...
                if (_vehicleRepository.IsVehicleAtWork((int)request.VehicleID) == true) return Task.FromResult(new CreateEmployeeCommandResponse(ResponseConstants.EmployeeCannotAssignToVehicle));

                if (_vehicleRepository.IsOwner((int)request.VehicleID, userID) == false) return Task.FromResult(new CreateEmployeeCommandResponse(ResponseConstants.NotVehicleOwner));
            }
